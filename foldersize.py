...
# Basisverzeichnis der Erweiterung
...
class FolderSize(GObject.GObject,
    ...
    def _evict_cache_if_needed(self):
        ...
        while len(FolderSize._cache) > MAX_CACHE_SIZE:
            try:
                oldest_path, _ = FolderSize._cache.popitem(last=False)
    ...
    def update_file_info(self, file):
        ...
        with FolderSize._cache_lock:
            if path in FolderSize._cache:
                # Move to end for LRU
                FolderSize._cache.move_to_end(path)
                ts, size, running, proc, queued, start_time = FolderSize._cache[path]
    ...
    def _enqueue_job(self, path, file, priority=False, force=False, override_disabled=False):
        if not FolderSize._scan_enabled and not override_disabled:
        ...
        if force:
            with FolderSize._cache_lock:
                if path in FolderSize._cache:
                    ts, size, running, proc, queued, st = FolderSize._cache[path]
        ...
        with FolderSize._cache_lock:
            FolderSize._cache[path] = (time.time(), "", False, None, True, None)
            self._evict_cache_if_needed()
        ...
        with FolderSize._queue_condition:
            with FolderSize._queue_lock:
                if priority:
                    FolderSize._queue.appendleft((path, file))
                else:
                    FolderSize._queue.append((path, file))
            FolderSize._queue_condition.notify()
    ...
    def _worker_loop(self):
        while True:
            ...
            with FolderSize._queue_condition:
                while True:
                    with FolderSize._queue_lock:
                        if FolderSize._queue:
                            path, file = FolderSize._queue.popleft()
                            break
                    # Wait for notification
                    FolderSize._queue_condition.wait(timeout=1.0)

            start_time = time.time()
            with FolderSize._cache_lock:
                FolderSize._cache[path] = (start_time, "", True, None, False, start_time)
            ...
            try:
                ...
                while True:
                    ...
                    with FolderSize._cache_lock:
                        FolderSize._cache[path] = (start_time, ICON_ACTIVE, True, proc, False, start_time)
            ...
            with FolderSize._cache_lock:
                FolderSize._cache[path] = (time.time(), size, False, None, False, None)

            # Update UI
            target_file = file
            if not target_file:
                with FolderSize._file_refs_lock:
                    target_file = FolderSize._file_refs.get(path)
                    FolderSize._file_refs.pop(path, None)
        ...
        with FolderSize._cache_lock:
            for path, (ts, size, running, proc, queued, start_time) in list(FolderSize._cache.items()):
                if running and start_time and (now - start_time) > LONG_JOB_AFTER:
                    ...
                    FolderSize._cache[path] = (ts, new_symbol, running, proc, queued, start_time)
        ...
        if stuck_jobs:
            with FolderSize._file_refs_lock:
                for path in stuck_jobs:
                    ...
                    with FolderSize._queue_condition:
                        with FolderSize._queue_lock:
                            FolderSize._queue.appendleft((path, ref_file))
                        FolderSize._queue_condition.notify()
    ...
    def get_file_items(self, *args):
        ...
        # Nautilus reicht entweder nur files oder (window, files)
        files = args[0] if len(args) == 1 else args[1]

        # Nur Verzeichnisse herausfiltern
        dir_files = []
        for f in files:
            try:
                if f.is_directory():
        ...
        if dir_files:
            # Singular/Plural anhand der Anzahl der Verzeichnisse
            ...
            # nur die Verzeichnisse an den Callback übergeben
